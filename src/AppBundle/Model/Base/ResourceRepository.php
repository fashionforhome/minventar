...
/**
 * Base class of repository of Model\Resource document.
 */
abstract class ResourceRepository extends \Mandango\Repository
{
    /**
     * {@inheritdoc}
     */
    ...
    {
        $this->documentClass = 'Model\Resource';
        $this->isFile = false;
        $this->collectionName = 'resource';
    ...
    public function fixMissingReferences($documentsPerBatch = 1000)
    {
        $skip = 0;
        do {
            $cursor = $this->getCollection()->find(array('type' => array('$exists' => 1)), array('type' => 1))->limit($documentsPerBatch)->skip($skip);
            $ids = array_unique(array_values(array_map(function ($result) { return $result['type']; }, iterator_to_array($cursor))));
            if (count($ids)) {
                $collection = $this->getMandango()->getRepository('Model\ResourceType')->getCollection();
                ...
                if ($idsDiff = array_diff($ids, $referenceIds)) {
                    $this->remove(array('type' => array('$in' => $idsDiff)), array('multiple' => 1));
                }
            }
        ...
        $skip = 0;
        do {
            $cursor = $this->getCollection()->find(array('resources' => array('$exists' => 1)), array('resources' => 1))->limit($documentsPerBatch)->skip($skip);
            $ids = array_unique(array_reduce(
                array_values(array_map(function ($result) { return $result['resources']; }, iterator_to_array($cursor)))
            , 'array_merge', array()));
            if (count($ids)) {
                $collection = $this->getMandango()->getRepository('Model\Resource')->getCollection();
                $referenceCursor = $collection->find(array('_id' => array('$in' => $ids)), array('_id' => 1));
                $referenceIds =  array_values(array_map(function ($result) { return $result['_id']; }, iterator_to_array($referenceCursor)));

                if ($idsDiff = array_diff($ids, $referenceIds)) {
                    $this->update(array(), array('$pull' => array('resources' => array('$in' => $idsDiff))), array('multiple' => 1));
                }
            }
